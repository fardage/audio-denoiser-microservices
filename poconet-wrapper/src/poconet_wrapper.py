...
def wav_read(wav_name):
    with wave.open(wav_name, "rb") as wav:
        if wav.getsampwidth() != 2:
            raise RuntimeError(
                "wav file {} does not have int16 format".format(wav_name)
            )
        if wav.getframerate() != 16000:
            raise RuntimeError(
                "wav file {} does not have 16kHz sampling rate".format(wav_name)
            )
...
def run_enhancer(
    input,
    output,
    model="./models/FP32/noise-suppression-poconetlike-0001.xml",
    device="CPU",
):
    log.basicConfig(
    ...
    log.info(
        "state_param_num = {} ({:.1f}Mb)".format(
            state_param_num, state_param_num * 4e-6
        )
    )

    # load model to the device
    log.info("Loading model to the {}".format(device))
    ie_encoder_exec = ie.load_network(network=ie_encoder, device_name=device)

    sample_inp = wav_read(input)
    ...
    log.info(
        "Sequence of length {:0.2f}s is processed by {:0.2f}s".format(
            sum(s.shape[0] for s in samples_out) / 16000,
            sum(samples_times),
        )
    )
    sample_out = np.concatenate(samples_out, 0)
    wav_write(output, sample_out)
